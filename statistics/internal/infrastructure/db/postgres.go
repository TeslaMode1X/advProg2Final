...
import (
	...
	"github.com/TeslaMode1X/advProg2Final/statistics/internal/repository/dao"
...
func (p *postgresDatabase) Migrate() {
	if err := p.DB.Migrator().AutoMigrate(&dao.RecipeReviewStatisticsEntity{}); err != nil {
		log.Fatalf("error migrating database: %v", err)
	}

	if err := p.DB.Migrator().AutoMigrate(&dao.UserStatisticsEntity{}); err != nil {
		log.Fatalf("error migrating database: %v", err)
	}
}
