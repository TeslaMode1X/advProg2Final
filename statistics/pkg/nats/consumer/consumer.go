...
import (
	"context"
	"fmt"
	natscl "github.com/TeslaMode1X/advProg2Final/statistics/pkg/nats"
	"github.com/TeslaMode1X/advProg2Final/statistics/pkg/nats/safe"
...
func (c *PubSub) Subscribe(cfg ...PubSubSubscriptionConfig) {
	log.Println("Subscribing to NATS")
	log.Println(cfg)
...
func (c *PubSub) consume(cfg PubSubSubscriptionConfig) error {
	...
	if err != nil {
		return fmt.Errorf("c.users.Subscribe: %w", err)
	}
